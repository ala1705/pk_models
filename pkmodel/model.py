...
class Model:
    ...
    def __init__(self, clearance_rate: float = 1.0, X: float = 1.0, dose: int = 0, 
        ...
        :param X: The constant dose (ng) into the central compartment
        :param dose: The number of time-steps the drug is administered at a time (if dose = 0, 
        there is single dose at time 0)
        If no_dose = 0, the dose can be 0 (instantaneous dose) or 1 (continuous dose)
        :param no_dose: The number of time-steps for which drug is not 
        ...
        if isinstance(clearance_rate, (float, int)) and isinstance(X, (float, int)) \
                and isinstance(V_c, (float, int)):
            if clearance_rate >= 0 and X >= 0 and V_c > 0:
        ...
        self.subdiv = self.num_timesteps
